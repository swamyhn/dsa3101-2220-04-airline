    ...
    tabPanel("Vis 1",
             sidebarLayout(
               ...
               mainPanel(
                 ...
                 fluidRow(
                   column(width = 12, plotOutput("selected_plot"), style = "padding-bottom: 15px;"),
...
server <- function(input, output) {
  ...
  observeEvent(input$plot_button, {
    if (input$model == "Linear Model") {
      ...
      output$plot1 <- renderPlot({
        coefficients$Variables <- factor(coefficients$Variables, levels = rev(coefficients$Variables[order(coefficients$Coefficients)]))

        # Assign colors based on positive/negative correlation
        coefficients_processed <- coefficients %>% mutate(colour = ifelse(Coefficients > 0, "#619CFF", "#F8766D"))
          ggplot(coefficients_processed, aes(x = Coefficients, y = Variables, fill = colour)) +
            ...
            scale_fill_manual(name = "Legend", values=c("#F8766D", "#619CFF"), labels=c("negative correlation","positive correlation")) +
            ggtitle("Plot with standardised coefficients (T)")
      })

        output$plot2 <- renderPlot({
        coefficients2$Variables <- factor(coefficients2$Variables, levels = rev(coefficients2$Variables[order(coefficients2$Coefficients)]))

        # Assign colors based on positive/negative correlation
        coefficients_processed <- coefficients2 %>% mutate(colour = ifelse(Coefficients > 0, "#619CFF", "#F8766D"))
          ggplot(coefficients_processed, aes(x = Coefficients, y = Variables, fill = colour)) +
            geom_bar(stat = "identity") +
            theme(axis.title = element_text(size = 17),
                  plot.title = element_text(size = 18),
  ...
  reactive_text3 <- eventReactive(input$plot_button, {
    if (input$model == "Linear Model") {
      paste0("Barplot of correlation of variables in relation to ", input$flight, " flights in Year ", input$year3, " (", input$model,")")
    } else {
      paste0("Barplot of correlation of variables in relation to ", input$flight, " flights in Year ", input$year3, " (", input$model," Model)") 
    }
  })
