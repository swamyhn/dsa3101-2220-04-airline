...
for year in years:
    ...
    cur.execute(f"""LOAD DATA INFILE '{file_path}' INTO TABLE airlines.year_{year} FIELDS TERMINATED BY ',' 
    ...
    snwd_origin = NULLIF(@snwd_origin, ''), arr_delay = NULLIF(@arr_delay, ''), 
    arr_delay_group = NULLIF(@arr_delay_group, ''), snwd_dest = NULLIF(@snwd_dest, '');""")
    print(f'{year} LOADED')
